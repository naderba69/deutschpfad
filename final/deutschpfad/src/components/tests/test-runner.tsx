...
export function TestRunner({ kind, level, sections, questions, title, resetKey , onComplete }: TestRunnerProps) {
  ...
  const setAnswer = (value: string | number | null) => {
    if (!current) return;
    setAnswers((prev) => {
      ...
      <div className="rounded-xl border bg-card p-4">
        ...
        {current.kind === "writing" ? (
          ...
          <>
            ...
            {current.options && (
              <div className="mt-4 grid gap-2 sm:grid-cols-2" role="radiogroup" aria-label={current.questionDe ?? "سؤال"}>
