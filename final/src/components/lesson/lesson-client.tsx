...
import {ArrowLeft, ArrowRight, Award, CheckCircle2, Clock, ListChecks, Printer, Sparkles, TableProperties} from "lucide-react";
    ...
    <div className="mt-10 rounded-2xl border bg-muted/10 p-5">
      <div className="flex flex-col items-center gap-3 text-center sm:flex-row sm:justify-between sm:text-start">
        ...
        <div className="flex shrink-0 flex-col items-center gap-2 sm:items-end">
          ...
          {done && (
            <div className="mt-3 grid w-full gap-2 rounded-xl bg-card/60 p-3 sm:grid-cols-3">
              <div className="text-center">
                <p className="font-de text-lg font-extrabold text-primary">{lesson.theory.length}</p>
                <p className="text-[10px] font-bold text-muted-foreground">كتل نظرية (قواعد + تريكات)</p>
              </div>
              <div className="text-center">
                <p className="font-de text-lg font-extrabold text-primary">{lesson.practiceBank.length + lesson.writing.length}</p>
                <p className="text-[10px] font-bold text-muted-foreground">تمريناً تفاعلياً</p>
              </div>
              <div className="text-center">
                <p className="font-de text-lg font-extrabold text-success">{lesson.flashcards.length}</p>
                <p className="text-[10px] font-bold text-muted-foreground">بطاقة مراجعة</p>
              </div>
            </div>
          )}
...
/**
 * مؤشر موضع الدرس في المسار الكامل: «الدرس 12 من 46».
 */
...
export function LessonClient({ lesson }: { lesson: Lesson }) {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-4 py-8 sm:px-6">
      ...
      {/* متصفح الأقسام العشرة — شريط علوي لاصق (يُخفى في وضع التركيز) */}
      {!focusMode && (
      ...
      {stepMode ? (
        /* ═══ التدفق التفاعلي الحي: المحتوى يتكشف مع التفاعل ═══ */
        <LessonFlow
          lesson={lesson}
          onFinish={() => {
