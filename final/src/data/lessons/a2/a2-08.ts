  ...
  theory: [
    {
      ...
      explanationAr:
        "المقارنة: صفة + er + als: Der Zug ist schneller als der Bus (أسرع من). التفضيل: am + صفة + sten: Der Zug ist am schnellsten (الأسرع). الشواذ: gut → besser → am besten، viel → mehr → am meisten، gern → lieber → am liebsten، groß → größer → am größten (مع Umlaut). ابدأ بالتمييز بين ثلاث وظائف: الصفة العادية تصف شيئاً واحداً (Der Zug ist schnell)، والمقارنة تضع شيئين في علاقة غير متساوية (Der Zug ist schneller als der Bus)، والتفضيل يختار أعلى درجة داخل مجموعة (Der Zug ist am schnellsten). في الصفات القصيرة نضيف -er و-sten، لكن وجود Umlaut في بعض الصفات ليس اختيارياً: alt يصبح älter وgroß يصبح größer، بينما schnell وbillig لا يغيران حرف العلة. عند استعمال الصفة قبل اسم ستتغير نهايتها بسبب تصريف الصفات، لكن هذا الدرس يركز على استعمالها بعد sein، لذلك نقول Der Zug ist schnell وليس schnellِر كصفة مستقلة. في التفضيل بعد أداة تعريف يمكن استعمال der schnellste Zug، أما في الجملة الخبرية فنستخدم am schnellsten. كذلك لا نضيف am إلى المقارنة: schneller هو «أسرع»، وam schnellsten هو «الأسرع». أما als فتستعمل للمقارنة غير المتساوية، وwie بعد so ... wie للمساواة. هذه الفروق تمنع ترجمة كل «من/مثل» بكلمة واحدة، وتبني أساساً لتبرير اختيار وسيلة نقل في محادثة حقيقية.",
      whyAr:
    ...
    },
    {
      ...
      explanationAr:
        "وسائل النقل: der Zug (قطار)، der Bus (حافلة)، die U-Bahn (مترو)، die Straßenbahn (ترام)، das Taxi (تاكسي)، das Fahrrad (دراجة). عبارات التذاكر: Eine Fahrkarte nach Berlin, bitte! (تذكرة إلى برلين)، Einfach (ذهاب) أو Hin und zurück (ذهاب وعودة)، am Automaten (من الجهاز)، am Schalter (من الشباك). في الموقف الحقيقي لا تحفظ الكلمات منفصلة فقط؛ اقرأ المعلومات في تسلسل: الوجهة، نوع التذكرة، وقت الانطلاق، الرصيف، ثم عدد مرات التغيير. nach تستعمل عادة مع المدن والبلدان غير المصحوبة بأداة، بينما zu تتجه غالباً إلى شخص أو مكان محدد: Ich fahre nach Berlin، لكن Ich gehe zum Bahnhof. الفعل المنفصل يظل مصرفاً في المركز الثاني وتذهب بادئته إلى النهاية: Der Zug fährt um zehn Uhr ab. ومع فعل ناقص يبقى المصدر في النهاية: Ich muss in Hamburg umsteigen. وعند المقارنة لا تكفي السرعة؛ يمكن أن تكون وصلة أرخص أو أهدأ أو مباشرة، ولذلك نحتاج عبارات تبرير مثل Für mich ist die direkte Verbindung besser. هذا يحول المفردات إلى قدرة تواصلية: شراء تذكرة، سؤال موظف، فهم إعلان، واتخاذ قرار معلل، لا مجرد حفظ أسماء وسائل النقل.",
      whyAr:
