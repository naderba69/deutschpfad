...
export function scoreWriting(q: TestQuestion, answer: string): { correct: boolean; pct: number } {
  ...
  const goethePct = evaluateWriting(answer, "Forumsbeitrag").pct;
...
export function scoreSkill(
  ...
  for (const q of questions) {
    ...
    if (q.kind === "mcq") {
      ...
      const s = scoreWriting(q, typeof ans === "string" ? ans : "");
...
export function buildTestResult(input: {
  ...
  const skillResults = skills
    .map((s) => scoreSkill(s, input.questions, input.answers))
    .filter((r) => r.total > 0);
